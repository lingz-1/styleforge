...
> 更新时间：2026-09-01
> 2026-09-01 **评估计划 v2 已重新制定，尚未执行新实验**：评估对象切换为当前六任务 Multi-Agent Harness，旧 p-outfit order/image runner 仅保留历史证据；重新冻结 L1 硬合同、L2 Agent/工具轨迹、L3 五维质量/稳定性/性能、L4 系统基线/消融/官方 Compatibility/FITB 四层结构。第一开发批次只实现官方 Polyvore Outfits 的 Random、Category Co-occurrence、FashionCLIP 离线基线，不恢复 legacy、不训练新模型、不扩 Agent/MCP/UI。详见 [评估计划 v2](EVALUATION_PLAN_V2.md)。
...
> 2026-08-29 **真实个人衣橱生产闭环 v1 完成**：批量识别从进程内状态迁移到 PostgreSQL Schema v14（`recognition_batches` / `recognition_batch_items`），原图原子暂存到项目 artifacts；支持 API 重启恢复、失败识别重试、向量失败独立重试、用户隔离预览与终态清理。逐图使用确定性 UUID，模拟“衣物已保存但批次状态未提交”的崩溃恢复不会重复建衣物。个人向量新增输入指纹；换图/改元数据先失效旧向量，非 `ready` 向量绝不参与检索；个人图片嵌入和查询检索共用进程级线程安全 FashionCLIP 运行时，避免重复加载 GPU 权重。Web 刷新后仍可查看暂存原图、编辑入库或重试。全量 Pytest **769 passed**（最终回归见本次记录）、CI Ruff、Vite 构建和真实浏览器 E2E 通过；11 张公开 Polyvore 图片 CUDA 验收 11/11，真实 DeepSeek 六类任务最终 7/7，报告 `artifacts/evaluation/wardrobe_quality_real_six_task_v14_final.json`。
> 2026-08-29 **真实衣物照片入库后的检索闭环完成**：批量识别不再永久停在 `embedding_status=pending`；可靠单品全部入库后，由独立单线程任务一次性生成个人 FashionCLIP 增量向量，避免三个识别 worker 同时加载 GPU 模型。批次状态扩为 `running → embedding → completed`，向量失败保留已入库衣物并返回稳定错误码、可重试标志，可通过原批次重试而不重复调用视觉识别或重复建衣物；运行中批次禁止删除。即使 API 重启导致内存批次丢失，新增 `/wardrobes/{user_id}/embeddings/retry` 仍可从 PostgreSQL 找出该用户至多 200 件 `pending/failed` 衣物补齐向量，提交其他用户 ID 不会越权。Web 衣柜页显示向量状态和一键补齐入口。定向回归 **10 passed**、全量 **762 passed**，Ruff clean，Vite 生产构建和浏览器两种重试路径 E2E 通过。另用 Polyvore 11 张真实商品图在 CUDA 上完成实际验收：11/11 写入 PostgreSQL `personal_item_embeddings`，均为 512 维归一化 `image` 向量、单品状态均为 `ready`；4 条中英文混合检索全部包含预期商品，测试数据已清理。
> 2026-08-29 **Prometheus/Grafana 本地监控栈完成并真实运行**：新增 `deploy/monitoring`，固定使用 Prometheus v3.5.5（LTS patch）与 Grafana 13.1.0；Prometheus 通过动态 file-SD 抓取可配置 API 端口，15 秒采集、15 天保留，7 条规则覆盖 API 下线、HTTP 高失败率/高延迟、数据库失败、连续 LLM 失败、重试风暴与降级执行。Grafana 自动预置 Prometheus 数据源和 9 面板“StyleForge 系统运行”仪表盘；所有端口只绑定回环地址，数据写入 `artifacts/monitoring`。统一 `start_monitoring.ps1` 支持 Start/Stop/Status/Check、独立 Compose/插件双兼容和端口参数。真实验收：`promtool` 配置成功、7/7 rules 成功、容器到 `/metrics` 连通、Prometheus target `up=1`、指标入库、Grafana API/数据库/9面板正常、无头 Edge 登录与仪表盘断言通过；全量 Pytest **758 passed**、Ruff clean。当前机器因其他项目占用 8000/3000，StyleForge 实际运行于 API 18000/Web 15173，Grafana 使用 3300；未干扰其他容器。
> 2026-08-29 **Prometheus 指标与系统健康面板完成**：新增依赖无关的 `/metrics` Prometheus text exposition 0.0.4，覆盖 HTTP 请求、任务/Agent/LLM/工具/数据库操作、重试/降级、耗时及按稳定错误码/组件聚合的错误计数；敏感请求内容和关联 ID 不进入指标。Web 新增 `/health`“系统脉搏”页，以 `HTTP → Task → Agent → LLM → Tool → PostgreSQL` 链路呈现进程内状态，并展示 PostgreSQL、语义检索、天气、图像源、目录来源与错误分布，支持 15 秒自动刷新和移动端。验证：观测测试 **9 passed**、全量 Pytest **754 passed**、Ruff clean、Vite 生产构建通过；健康页桌面/390px E2E 与项目完整浏览器 E2E 均通过，完整 E2E 产出 2 套推荐、10 次混合检索、0 降级、浏览器控制台无错误。
> 2026-08-29 **错误可观测性与故障注入完成**：新增进程级线程安全观测注册表，贯通 `request_id → run_id → task/agent/llm/tool/database`；结构化 JSON 错误事件不记录用户原话、工具参数或异常正文。`/health.observability` 暴露请求失败率、各操作调用/失败/可重试失败/重试/降级计数及平均/最大耗时，错误按稳定 code 与组件聚合；明细事件只保留在内部有界队列，不通过健康接口暴露。故障注入覆盖 HTTP 上游超时与断网、DeepSeek 不可用、工具超时、数据库事务回滚；全量 Pytest **752 passed**，Ruff 全通过。
> 2026-08-29 **统一错误处理与推荐意图门完成**：FastAPI、Agent Runtime、Tool Runtime、工作流、数据库事务、Web 和小程序统一使用稳定错误码、`request_id`、`retryable` 与安全消息；保留兼容字段 `detail`，未知异常不再向客户端泄露内部错误。新增推荐专用的确定性意图门，校验久站舒适、防水、防滑、保暖、正式、篮球运动/缓震等有权威衣橱元数据支撑的明确要求；不用于修改任务，旧衣橱缺少 features 时安全降级给 Critic。普通通勤和未注明户外的篮球不再误入事件 Research/默认城市天气链。真实 DeepSeek 三条关键推荐复测 **3/3**，调用量分别 12/10/6，五维分与槽位/特征全部达标；全量 Pytest **745 passed**，Ruff 全通过。报告：`artifacts/evaluation/wardrobe_quality_configured_recommend_intent_gate_network.json`。
> 2026-08-28 **真实六任务质量基线完成**：固定质量集为 60 条（42 条无 LLM/GPU 日常门禁 + 18 条 configured 合同案例），另建 7 条 Polyvore 官方商品真实用例，覆盖推荐、修改、风格建议、单品建议、兼容性和衣橱缺口；DeepSeek 验收 7/7，路由/状态准确率 100%。Main Graph 现把 `MultiTaskWorkflow` 的任务类型作为权威路由，补齐 `one_piece + footwear`、通用 `accessory` 物理结构，以及模型协议失败/严重意图错位时的衣橱内 grounded recovery；恢复候选仍必须经过环境门禁与 Critic。当前全量 Pytest **736 passed**，轻量质量基准 42/42，正式 CI Ruff 范围通过。报告：`artifacts/evaluation/wardrobe_quality_real_six_task_final.json`。
...
| P5 天气上下文工具 + MCP | 已实现，协议测试通过 | Agent 1 按需声明天气；typed Weather Tool 经官方 Time/Fetch MCP 获取 Open-Meteo 事实，失败回退直连；StyleForge 在 `/mcp/` 暴露五个工具；事实与去敏 MCP trace 进入 Context Pack、任务诊断和健康面板 |
...
> 构建了一个个人衣柜多 Agent 穿搭系统（默认配置 DeepSeek 语义链）。项目使用 FashionCLIP 生成多模态表示，通过 LangGraph 编排 Planner、Stylist 和 Reviewer，并使用确定性约束引擎保证推荐只引用用户衣柜中的可追溯商品。已对 126,928 张商品图完成全量 GPU 向量化和检索评估，并实现多受众商品目录与购物订单衣柜的可审计接入流程。
